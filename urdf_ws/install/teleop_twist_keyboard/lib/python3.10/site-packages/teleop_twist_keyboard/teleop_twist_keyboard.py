    ...
    def handle_keyboard_input(self):
        ...
        try:
            while True:
                key = get_key()
                if key == 'w':
                    self.publish_twist(1.0, 0.0)  # Avanzar
                elif key == 's':
                    self.publish_twist(-1.0, 0.0)  # Retroceder
                elif key == 'a':
                    self.publish_twist(0.0, 1.0)  # Girar izquierda
                elif key == 'd':
                    self.publish_twist(0.0, -1.0)  # Girar derecha
                elif key == ' ':
